# Needs Verification

Confirm these items with Slater Heating or a reliable public source before launch:

- Confirm email.
- Confirm hours.
- Confirm final towns and service availability within Greater Bangor and Central Maine.
- Confirm whether all listed services should appear publicly:
  - Oil heating systems.
  - Propane and natural gas heating systems.
  - Heating service.
  - Heating cleanings.
  - Heating installations.
  - Heat pump servicing and cleaning.
- Confirm logo permission.
- Confirm preferred family-owned wording and how Tim wants the business story presented.
- Confirm Google Business Profile/review link.
- Confirm permission to use as a portfolio example.
- Official business name and preferred spelling.
- Confirm brand colors.
- Confirm the names, relationship, and preferred wording for any family-business section.
- Mailing address or service address, if the owner wants it public.
- Whether emergency, after-hours, or same-day service is offered.
- Brands, system types, fuel types, and equipment specialties.
- Any additional certifications, license numbers, insurance wording, warranties, or guarantees the owner wants published.
- Final town list, including Bangor, Brewer, Eddington, Holden, Veazie, Clifton, Bradley, Milford, Old Town, and any other nearby communities.
- Google Business Profile link, rating, review count, and review-theme summary.
- Whether the website should include online forms, direct email, phone-only calls, or another lead workflow.
- Privacy/legal wording for the contact form.
